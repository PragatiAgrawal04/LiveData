...
if selected_option in share_list:
    ...
    for price in current_market_price(ticker, exchange):
        stock_ltp = price
        break
    st.write('Stock LTP:', stock_ltp)


    # ********************************** MATRIX ******************************************
    matrix = np.zeros((5, 4))
    df = pd.DataFrame(matrix, columns=["CE/LTP", "(STR-LTP)+CE/LTP", "PE/LTP", "(LTP-STR)+PE/LTP"])

    for i in range(len(df)):
        df.at[i, "CE/LTP"] = (output_ce["lastPrice"].iloc[i] / stock_ltp) * 100
        df.at[i, "(STR-LTP)+CE/LTP"] = (((output_ce["strikePrice"].iloc[i] - stock_ltp) + output_ce["lastPrice"].iloc[i]) / stock_ltp) * 100
        df.at[i, "PE/LTP"] = (output_pe["lastPrice"].iloc[i] / stock_ltp) * 100
        df.at[i, "(LTP-STR)+PE/LTP"] = (((stock_ltp - output_pe["strikePrice"].iloc[i]) + output_pe["lastPrice"].iloc[i]) / stock_ltp) * 100
    st.dataframe(df)
    # ************************************************************************************
